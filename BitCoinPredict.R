...
#load library packages as installed
library("anytime")
library("bsts")
library("car")
library("caret")
...
#importing data
train <- read.csv("C:/Users/swap/Documents/train.csv")
test <- read.csv("C:/Users/swap/Documents/test.csv")
#head function to show the sample data from the dataset
head(train)
testdata <- test[,2]

#Converting data for analysis in time series
...
#plotting the closing price of bitcoin yearwise
...
#Obtaining the trends and seasonality
dects <- decompose(tsr) 
plot(dects)
...
ggplot() + geom_line(data = holtfdf, aes(Date, holtfdf[,2]), color = "blue") + 
  geom_line(data = holtfdf, aes(Date, holtfdf[,3]), color = "Dark Red")
...
ss <- AddSeasonal(list(), Train[,4], nseasons = 365) #Adding seasonal trend to model
